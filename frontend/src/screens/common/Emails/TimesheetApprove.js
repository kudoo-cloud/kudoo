...
class TimesheetApprove extends React.Component {
  _renderHTML() {
    ...
    const approveUrl =
      window.location.origin + `/#/timesheets/${query.time_sheet_id}/approve`;
    return (
      <tr>
...
Select the link ${approveUrl} to approve the Timesheet.
        `}
      </React.Fragment>
    );
  }
  ...
  render() {
    ...
    return (
      <Common title='Timesheet Approve' type={type}>
        {() => {
          if (type === 'html') {
